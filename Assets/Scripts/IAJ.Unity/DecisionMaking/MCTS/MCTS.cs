...
namespace Assets.Scripts.IAJ.Unity.DecisionMaking.MCTS
{
    public class MCTS
    {
        ...
        public Action ChooseAction()
        {
            ...
            return this.BestAction(this.InitialNode);
        }
        ...
        protected MCTSNode Expand(MCTSNode parent)
        {
            ...
            // Go through all executable actions
            foreach (var action in executableActions)
            {
                bool alreadyTried = false;

                // Check if the action has already been tried (exists in one of the children nodes)
                ...
                // If the action hasn't been tried, add it to the untriedActions list
                if (!alreadyTried)
                {
        ...
        protected virtual void Backpropagate(MCTSNode node, float reward)
        {
            while (node != null)
            {
                ...
                {
                    node.Q -= reward; // opponent's perspective, minimize reward
                }
                else
                {
                    node.Q += reward; // maximizing player
                }
                node = node.Parent;
            }
        }
        ...
        protected MCTSNode BestChild(MCTSNode node)
        {
            ...
            foreach (var child in node.ChildNodes)
            {
                float childValue = child.Q / child.N;
